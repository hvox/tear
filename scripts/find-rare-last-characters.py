...
that are least likely to be before end-of-line characters.
"""
import sys
...
def process(path: Path) -> dict[str, int]:
    ...
    if path.is_dir():
        ...
        eprint(f"{path}: binary file, SKIPPP")
        return statistics
    elif ".git" in str(path):
        eprint(f"{path}: .git file, SKIP")
        return statistics
    elif "unity" in str(path):
        eprint(f"{path}: Unity file, SKIP")
        return statistics
    try:
        ...
        for line in (x for x in text.splitlines() if x):
            ...
            if char == "@":
                HNEATR[str(path)] = HNEATR.get(str(path), 0) + 1
